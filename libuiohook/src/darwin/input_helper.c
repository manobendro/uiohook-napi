...
// Layout-aware scancode remapping table.
// Maps macOS physical keycode (0-127) -> layout-aware VC scancode.
// A value of VC_UNDEFINED means "no remap, use static table".
...
static uint16_t char_to_vc_scancode(UniChar ch) {
    switch (ch) {
        case 'a': case 'A': return VC_A;
        case 'b': case 'B': return VC_B;
        case 'c': case 'C': return VC_C;
        case 'd': case 'D': return VC_D;
        case 'e': case 'E': return VC_E;
        case 'f': case 'F': return VC_F;
        case 'g': case 'G': return VC_G;
        case 'h': case 'H': return VC_H;
        case 'i': case 'I': return VC_I;
        case 'j': case 'J': return VC_J;
        case 'k': case 'K': return VC_K;
        case 'l': case 'L': return VC_L;
        case 'm': case 'M': return VC_M;
        case 'n': case 'N': return VC_N;
        case 'o': case 'O': return VC_O;
        case 'p': case 'P': return VC_P;
        case 'q': case 'Q': return VC_Q;
        case 'r': case 'R': return VC_R;
        case 's': case 'S': return VC_S;
        case 't': case 'T': return VC_T;
        case 'u': case 'U': return VC_U;
        case 'v': case 'V': return VC_V;
        case 'w': case 'W': return VC_W;
        case 'x': case 'X': return VC_X;
        case 'y': case 'Y': return VC_Y;
        case 'z': case 'Z': return VC_Z;

        case '0': case ')': return VC_0;
        case '1': case '!': return VC_1;
        case '2': case '@': return VC_2;
        case '3': case '#': return VC_3;
        case '4': case '$': return VC_4;
        case '5': case '%': return VC_5;
        case '6': case '^': return VC_6;
        case '7': case '&': return VC_7;
        case '8': case '*': return VC_8;
        case '9': case '(': return VC_9;

        case '-': case '_': return VC_MINUS;
        case '=': case '+': return VC_EQUALS;
        case '[': case '{': return VC_OPEN_BRACKET;
        case ']': case '}': return VC_CLOSE_BRACKET;
        case '\\': case '|': return VC_BACK_SLASH;
        case ';': case ':': return VC_SEMICOLON;
        case '\'': case '"': return VC_QUOTE;
        case '`': case '~': return VC_BACKQUOTE;
        case ',': case '<': return VC_COMMA;
        case '.': case '>': return VC_PERIOD;
        case '/': case '?': return VC_SLASH;
...
static void build_layout_scancode_table_impl(void *info) {
    ...
    if (input_source == NULL) {
        logger(LOG_LEVEL_WARN, "%s [%u]: Failed to get current keyboard layout input source.\n",
    ...
    if (layout_data == NULL) {
        logger(LOG_LEVEL_WARN, "%s [%u]: Failed to get unicode key layout data.\n",
                __FUNCTION__, __LINE__);
    ...
    if (keyboard_layout == NULL) {
        logger(LOG_LEVEL_WARN, "%s [%u]: Failed to get keyboard layout byte pointer.\n",
                __FUNCTION__, __LINE__);
        CFRelease(input_source);
        return;
    }

    UInt32 kbd_type = LMGetKbdType();

    // Iterate all physical keycodes (0-127) and translate each with no modifiers.
    for (UInt16 keycode = 0; keycode < LAYOUT_REMAP_TABLE_SIZE; keycode++) {
        ...
        if (status == noErr && actual_length > 0) {
            uint16_t vc = char_to_vc_scancode(chars[0]);
            if (vc != VC_UNDEFINED) {
                layout_scancode_table[keycode] = vc;
                logger(LOG_LEVEL_DEBUG, "%s [%u]: Remap keycode %u -> char '%c' -> scancode 0x%04X\n",
                        __FUNCTION__, __LINE__, keycode, (char)chars[0], vc);
            }
        }
    }
...
static void keyboard_layout_changed_callback(CFNotificationCenterRef center,
                                              void *observer,
                                              CFNotificationName name,
                                              const void *object,
                                              CFDictionaryRef userInfo) {
    logger(LOG_LEVEL_INFO, "%s [%u]: Keyboard layout changed, rebuilding remap table.\n",
            __FUNCTION__, __LINE__);
